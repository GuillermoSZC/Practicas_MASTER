...
struct Exercise3 {
	...
	static vec3 getWorldMousePosition(float mouse_x, float mouse_y, float windowsWidth, float windowsHeight, const mat4& projMat, const mat4& viewMat) {
		float x = ((mouse_x - windowsWidth) / windowsWidth) * 2.f + 1.f;
		float y = -(((mouse_y - windowsHeight) / windowsHeight) * 2.f + 1.f);
		// float z = -1.f;

		vec4 pos = vec4(x, y, -1, 1.f);
		vec4 viewPos = inverse(projMat) * pos;
		viewPos = vec4(viewPos.x, viewPos.y, -1.f, 0.f);

		vec4 inWorldPos = inverse(viewMat) * viewPos;
		normalise(inWorldPos);
	...
	static bool raySphereIntersection(const Ray& ray, vec3 C, float r, float* intersection_distance)
	{
		...
		vec3 vc1 = (C - origin); // C es la posicion de la cámara. vc1 es la posicion de la camara menos el origen del rayo
		vec3 vc2(cross(direction, vc1)); // dist entre el punto central de la esfera y el rayo
	...
	static void onKeyPressed(GLFWwindow* window, int key, int scancode, int action, int mods){
		...
		if (key == GLFW_KEY_0) {
			...
			Ray ray;
			ray.origin = mouseWorldPos + camPos;
			ray.direction = normalise(ray.origin);
	...
	static void onMouseClicked(GLFWwindow* window, int button, int action, int mods) {
		...
		if (GLFW_PRESS == action) {
			...
			const vec3 camPos = exercise.camNode.worldMatrix.getColumn(3);
			Ray ray;
			ray.origin = mouseWorldPos + camPos; // el origen del rayo es la posicion del raton cuando hacemos clik + camPos
			ray.direction = normalise(ray.origin - camPos); // la direccion del rayo va a ser el origen del rayo - camPos

			// check ray against all spheres in scene
			int closest_sphere_clicked = -1;
			exercise.axis.clear();

			Shapes::addArrow(exercise.axis, ray.origin, ray.origin + normalise(ray.direction), vec3(1, 0, 0)); // pinta la flecha
			exercise.axis.load_to_gpu();
