...
void Shapes::addArrow(Lines & lines, const vec3 & from, const vec3 & to, const vec3 & color)
{
	...
	vec3 shaft = to - from;
	vec3 minishaft = shaft * 0.9;
	float shaftLength = length(shaft);

	vec3 pp1 = cross(shaft, up);
	vec3 pp2 = cross(shaft, down);
	vec3 pp3 = cross(shaft, asd);

	pp1 = normalise(pp1);
	pp2 = normalise(pp2);
	pp3 = normalise(pp3);

	vec3 pointA = from + minishaft + pp1 * 0.1 * shaftLength;
	vec3 pointB = from + minishaft - pp1 * 0.1 * shaftLength;
	vec3 pointC = from + minishaft + pp2 * 0.1 * shaftLength;
	vec3 pointD = from + minishaft - pp2 * 0.1 * shaftLength;
	vec3 pointE = from + minishaft + pp3 * 0.1 * shaftLength;
	vec3 pointF = from + minishaft - pp3 * 0.1 * shaftLength;
	...
	unsigned int arrow_indices[] = {
		...
		6,1,
		7,1,
	}; // draw line from arrow_vertices[0] to arrow_vertices[1]
	
	lines.add(&arrow_vertices[0].v[0], &arrow_colors[0].v[0], 8, &arrow_indices[0], 14);
}
